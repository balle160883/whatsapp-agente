...
export class CalendarService {
  ...
  constructor(auth: OAuth2Client, calendarId: string, organizationId: string) {
    this.auth = auth
...
export async function getCalendarService(organizationId: string): Promise<CalendarService | null> {
  ...
  return new CalendarService(oauth2Client, config.calendarId ?? 'primary', organizationId)
}
