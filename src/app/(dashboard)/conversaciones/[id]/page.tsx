...
export default function ConversationDetailPage() {
  ...
  const fetchConversation = useCallback(async () => {
    try {
      ...
      if (!res.ok) { router.push("/conversaciones"); return; }
    ...
    try {
      ...
      if (res.ok) {
        setConversation((prev) =>
          prev ? { ...prev, botActive: !prev.botActive } : prev
        );
      }
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        <Link href="/conversaciones" className="btn btn-ghost btn-sm" style={{ padding: "0.375rem" }}>
          <ArrowLeft size={18} />
        </Link>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            {conversation.contact.isNewPatient && (
              <span className="badge badge-info" style={{ fontSize: "0.625rem" }}>Nuevo</span>
            )}
          </div>
        </div>
        ...
        {conversation.status === "HUMAN_HANDOFF" && (
          <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", color: "#f59e0b", fontSize: "0.8125rem" }}>
            <Warning size={16} />
            Requiere atención
          </div>
        )}
